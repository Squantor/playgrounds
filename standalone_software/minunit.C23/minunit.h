...
typedef struct {
   ...
   void (*testSetup)(
       minunitState *testResults); /*!< setup to run before the test */
   void (*testTeardown)(
       minunitState *testResults); /*!< teardown to run after the test */
} minunitTestEntry;
 ...
 * }
 * ```
 * @param[in] test Check to perform, must be an expression
 */
 ...
 */
#define MINUNIT_PASS() MU__SAFE_BLOCK(testResults->checks++;)

/**
 * \brief automatically fail test
 */
#define MINUNIT_FAIL()                                          \
   MU__SAFE_BLOCK(testResults->checks++; MinunitFailCallback(); \
                  testResults->flagFailed = 1; return;)
