...
public:
  ...
  template <typename T> inline bool expectEQ(const T &arg1, const T &arg2) {
    bool isTrue{arg1 == arg2};
    if (!isTrue) {
      std::cout << "Actual: " << arg1 << " vs. Expected: " << arg2 << std::endl;
    }
    return isTrue;
  }
...
#define DefineTest(Module, TestName) void Test_##Module##_##TestName::runFunc()

#define RegisterTest(Module, TestName)                                         \
  UnitTest::getInstance().testList.push_back(                                  \
      Test_##Module##_##TestName::getInstance());

#define ExpectEQ(arg1, arg2) isTrue &= expectEQ(arg1, arg2);

#define RunTests() UnitTest::getInstance().runTests();
