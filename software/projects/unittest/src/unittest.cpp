...
Log::Log(const char *module, const char *test, bool state) {
  ...
  if (state) {
    _stream << sUnit.str() + _sPassed.str();
  } else {
    _stream << sUnit.str() << _sFailed.str();
  }
}

Log::~Log() { std::cerr << _stream.str() << std::endl; }
...
Summary::Summary(const int &passed, const int &failed)
  ...
  _sPassed << "Tests Passed : "
           << "\x1b[38;5;40m" << passed << "\x1b[0m";
  _sFailed << "Tests Failed : "
           << "\x1b[38;5;160m" << failed << "\x1b[0m";
...
Summary::~Summary() {
  std::cerr << "\n"
            << _sBorder.str() << _sTotal.str() << total << "\n"
            << _sPassed.str() << "\n"
            << _sFailed.str() << "\n"
